...
import xarray as xr
import sys
n = pypsa.Network("elec_s_37_ec_lcopt_Co2L0.0-3H_minimum.nc")
...
for bus in ac_buses.index:
    n.add("Generator", f"SBSP Generator at {bus}", 
          ...
          capital_cost=396591.1043,  # EUR/MW
          p_nom_extendable=True  
         )

sbsp_limits = xr.open_dataset("sbsp_rd2_profile_2050.nc")
...
log_file = '2050_min_rd2_optimization_output.txt'
...
with open('2050_min_rd2_objective_constant.txt', 'w') as f:
    f.write(str(n.objective_constant))

#9
statistics = n.statistics().dropna()

statistics.to_csv('2050_min_rd2_statistics_cleaned.csv', index=True)

print("save to 'statistics_cleaned.csv'")

n.export_to_netcdf('optimized_2050_min_rd2_396591_network.nc')
n.stores.to_csv('stores_2050_min_rd2_396591.csv', index=False)
n.storage_units.to_csv('storage_units_2050_min_rd2_396591.csv', index=False)
n.generators.to_csv('generators_2050_min_rd2_396591.csv', index=False)
...
supply_data.to_csv('2050_min_rd2_hourly_energy_supply.csv', index=True)

print("2050_min_rd2_hourly_energy_supply.csv")
...
output_file = 'active_rd2_SBSP_buses_min.txt'
...
output_file = 'sbsp_rd2_p_nom_opt_results_min.txt'
with open(output_file, 'w') as f:
...
with open("total_cost_min_summary_rd2_396591.txt", "w") as f:
