...
for bus in ac_buses.index:
    n.add("Generator", f"SBSP Generator at {bus}", 
          ...
          capital_cost=4901645.439,  # EUR/MW
...
log_file = '2020_rd1_optimization_output.txt'
...
with open('2020_rd1_objective_constant.txt', 'w') as f:
    f.write(str(n.objective_constant))

#9
statistics = n.statistics().dropna()

statistics.to_csv('2020_rd1_statistics_cleaned.csv', index=True)

print("save to 'statistics_cleaned.csv'")

n.export_to_netcdf('optimized_2020_rd1_network.nc')
n.stores.to_csv('stores_2020_rd1.csv', index=False)
n.storage_units.to_csv('storage_units_2020_rd1.csv', index=False)
n.generators.to_csv('generators_2020_rd1.csv', index=False)
...
supply_data.to_csv('2020_rd1_hourly_energy_supply.csv', index=True)

print("2020_rd1_hourly_energy_supply.csv")
...
output_file = 'active_rd1_SBSP_buses.txt'
...
output_file = 'sbsp_rd1_p_nom_opt_results.txt'
with open(output_file, 'w') as f:
...
with open("total_cost_summary_rd1.txt", "w") as f:
